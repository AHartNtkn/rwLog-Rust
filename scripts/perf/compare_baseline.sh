...
if [[ -z "$name" ]]; then
  ...
  echo "Env: RWLOG_PERF_SKIP_DIFF=1 to skip perf_corpus_diff" >&2
  exit 2
fi
shift || true
...
if [[ "$shards_raw" == "all" ]]; then
  echo "==> comparing against baseline '${name}' in single process"
  cargo bench --bench perf_corpus -- --baseline "$name" "${criterion_args[@]}"
else
  ...
  for raw_shard in "${shards[@]}"; do
    ...
    echo "==> comparing baseline '${name}' shard='${shard}'"
    if ! cargo bench --bench perf_corpus -- "$shard" --baseline "$name" "${criterion_args[@]}"; then
      failed+=("$shard")
    fi
  done

  if (( ${#failed[@]} > 0 )); then
    echo "baseline compare failed for shard(s): ${failed[*]}" >&2
    exit 1
  fi
fi

if [[ "${RWLOG_PERF_SKIP_DIFF:-0}" == "1" ]]; then
  exit 0
fi

threshold="${RWLOG_REGRESS_THRESHOLD_PCT:-5}"
cargo run --release --bin perf_corpus_diff -- --threshold-pct "$threshold"
