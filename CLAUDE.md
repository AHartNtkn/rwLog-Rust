...
The core rewriting engine uses **matching**, not unification. When composing or intersecting pattern spans (RwL/RwR), the two sides have **separate substitutions** and disjoint variable namespaces. Variable identities are local to each side; the same variable index on both sides has no shared meaning. Any matching algorithm must rename apart (or otherwise guarantee disjoint variable namespaces) before attempting to relate two sides.

Matching must be invariant under renaming variables on only one side, and property tests should assert this.

**Exception:** The CHR constraint system's `=` builtin performs true unification (`unify_into`) with a single shared substitution. This is correct and intentional — CHR equality constraints unify their arguments.

**Rule of thumb:** If you're working on span composition, intersection, or factoring, you need matching (two-sided substitutions). If you're working on CHR constraint solving, unification (single substitution) is appropriate.
