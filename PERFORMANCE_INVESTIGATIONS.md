...
10. Add object pool / free list for Box<Work> and Box<Node> to eliminate 880K malloc/free cycles per query. **Investigated** — see [docs/perf_investigations/allocation_overhead_analysis.md](docs/perf_investigations/allocation_overhead_analysis.md).
...
10. Allocation overhead reduction (Box<Work>/Box<Node> size + malloc/free elimination). **Investigated** — see [docs/perf_investigations/allocation_overhead_analysis.md](docs/perf_investigations/allocation_overhead_analysis.md). Originally 968K allocations (390MB). After Box PipeWork (Option A) and FixWork in-place stepping: 752K allocations (179MB). Remaining: 223K Box<Work> (non-Fix variants) + 444K Box<Node>. Object pool/arena for 280B and 232B size classes is the next avenue.
