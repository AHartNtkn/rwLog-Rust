...
2. ~~Introduce immutable arena-backed nodes for `Rel`/`Node` to reduce `Arc` churn.~~ **Partially addressed — ~19% improvement.** See [docs/perf_investigations/box_emit_node_shrink.md](docs/perf_investigations/box_emit_node_shrink.md). Boxing NF in Node::Emit shrank Node from ~136B to ~24B, eliminating most memcpy (11.58% → target) and drop (6.17% → target) overhead. Arena-backed nodes may yield further improvement but the dominant cost (Node size) has been addressed.
